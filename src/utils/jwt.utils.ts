...
const TOKEN_EXPIRY = process.env.TOKEN_EXPIRY || '15m' // 15 minutes for access token
const REFRESH_TOKEN_EXPIRY = process.env.REFRESH_TOKEN_EXPIRY || '7d' // 7 days for refresh token
...
const refreshTokenStore: Record<
  string,
  {
...
export class JwtUtils {
   ...
   */
  static async generateTokens(user: Partial<User>): Promise<{
    accessToken: string
    refreshToken: string
  }> {
  ...
  static async refreshAccessToken(refreshToken: string): Promise<{
    ...
    const accessToken = await jwtInstance.decorator['auth-token'].sign({
      id: userId,
    })
