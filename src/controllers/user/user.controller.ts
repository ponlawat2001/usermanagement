  ...
  .post(
    ...
    async ({ body, set }) => {
      try {
        ...
        const existingUser = await userService.findByUsernameOrEmail(body.username)
        if (existingUser) {
          throw new Error('Username already exists')
        }

        const existingEmail = await userService.findByUsernameOrEmail(body.email)
        if (existingEmail) {
  ...
  .patch(
    ...
    async ({ params, body, set }) => {
      try {
        ...
        if (body.email) {
          const existingUserWithEmail = await userService.findByUsernameOrEmail(body.email)
          if (existingUserWithEmail && existingUserWithEmail.email === body.email) {
            throw new Error('Email already exists')
          }
        }
